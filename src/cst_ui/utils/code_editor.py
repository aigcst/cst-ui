...
class GitHubDark:
    ...
    comment: str = "#0D1117"  # muted gray for comments
...
class CodeEditor(ft.Container):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.font = "AlibabaSans-Regular"

        self.line_number_width = 30
        ...
        self.syntax_rules = {
            "python": {
                "keywords": (
                    r"\b(?P<KEYWORD>False|None|True|and|as|assert|async|await|break|class|continue|def|del|elif|else|"
                    r"except|finally|for|from|global|if|import|in|is|lambda|nonlocal|not|or|pass|"
                    r"raise|return|try|while|with|yield)\b",
                    self.theme.keyword,
                ),
                "exceptions": (
                    r"([^.'\"\\#]\b|^)(?P<EXCEPTION>ArithmeticError|AssertionError|AttributeError|BaseException|"
                    ...
                    r"RuntimeWarning|StopAsyncIteration|StopIteration|SyntaxError|SyntaxWarning|SystemError|SystemExit|"
                    r"TabError|TimeoutError|TypeError|UnboundLocalError|UnicodeDecodeError|UnicodeEncodeError|"
                    r"UnicodeError|UnicodeTranslateError|UnicodeWarning|UserWarning|ValueError|Warning|"
                ...
                "docstrings": (
                    r"(?P<DOCSTRING>(?i:r|u|f|fr|rf|b|br|rb)?'''[^'\\]*((\\.|'(?!''))[^'\\]*)*(''')?|(?i:r|u|f|fr|rf|"
                    r"b|br|rb)?\"\"\"[^\"\\]*((\\.|\"(?!\"\"))[^\"\\]*)*(\"\"\")?)",
        ...
        self.code_textfield_container = ft.Container(
            ...
            padding=ft.padding.only(left=self.line_number_width + 5, top=5),
            width=800,
            height=self.height,
        )
        ...
        if self.read_only:
            self.content = ft.Stack(controls=[self.code_highlight_container])
            self.padding = 10
            self.bgcolor = self.theme.editor_bg
    ...
    def apply_syntax_highlighting(self, text, language):
        ...
        for idx, line in enumerate(lines):
            ...
            line_number = ft.Container(
                ft.Text(f"{idx + 1}", color="#60676f"), width=self.line_number_width
            )
